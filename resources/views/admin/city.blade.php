      ...
      <table class="table border">
        ...
        <tbody > 
          ...
          @foreach ($data as $item)
           <tr>
              <td>{{ $id++ }}</td>
              <td> <a href="/neighborhood/{{ $item->id }}">{{ $item->name_ar }}</a>  </td>
              <td>
                <!-- <a href="#" data-id="{{ $item->id }}" class="btn-action edit"  data-toggle="modal" data-target="#EditModal"> <i class="fas fa-pencil-alt"></i> </a> -->
                <a href="/delete.city/{{ $item->id }}"  class="btn-action"> <i class="far fa-trash-alt"></i> </a>
              </td>
            </tr>
          @endforeach
        </tbody>
...
<div class="modal fade" id="newModal" tabindex="-1" role="dialog" aria-labelledby="newModalLabel" aria-hidden="true">
  <form class="modal-dialog" role="document" action="{{ route('store.city') }}" method="POST" enctype="multipart/form-data">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-outline">
            ...
            <label class="form-label" for="formControlLg" style="margin-left: 0px;"> اسم المدينة </label>
